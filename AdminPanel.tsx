...
import {
  ...
  getRuntimeHealth,
  getRuntimeReadiness,
  updateAdminAgentVoiceAssignment,
  HyperCocreatorMe,
  RuntimeHealth,
  RuntimeReadiness,
  TeamDefinition,
...
const NAV: Array<{ id: AdminView; label: string; kicker: string }> = [
  ...
  { id: "runtime", label: "Runtime", kicker: "Saúde" },
...
export default function AdminPanel() {
  ...
  const [voiceAgentSlug, setVoiceAgentSlug] = useState("");
  const [voiceCatalogId, setVoiceCatalogId] = useState("");
  const [voiceLocale, setVoiceLocale] = useState("pt-BR");
  const [voicePresentation, setVoicePresentation] = useState<"MASCULINA" | "FEMININA" | "NEUTRA" | "NAO_DEFINIDA">("NAO_DEFINIDA");
  const [voiceSaving, setVoiceSaving] = useState(false);
  const [voiceNotice, setVoiceNotice] = useState("");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(true);
  const [domainState, setDomainState] = useState<Record<
    "overview" | "users" | "agents" | "teams" | "voices" | "runtime" | "security" | "governance",
    DomainState
  >>({
    overview: "loading",
    users: "loading",
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const [overview, userRows, catalog, teamCatalog, voiceRows, assignmentRows, runtimeHealthStatus, runtimeReadyStatus, securityStatus, governanceStatus] =
        ...
        setDomainState({
          ...
          runtime: runtimeHealthStatus.status === "fulfilled" && runtimeReadyStatus.status === "fulfilled" ? "ok" : "unavailable",
        ...
        if (runtimeHealthStatus.status === "fulfilled") setRuntimeHealth(runtimeHealthStatus.value);
        if (runtimeReadyStatus.status === "fulfilled") setRuntimeReadiness(runtimeReadyStatus.value);
  ...
  const selectedVoice = voiceCatalog.find((voice) => voice.id === voiceCatalogId) || null;
  const eligibleVoices = voiceCatalog.filter(
    (voice) => voice.active && voice.curation_status === "APPROVED",
  );

  const createVoiceDraft = async () => {
    setVoiceNotice("");
    if (!voiceAgentSlug || !selectedVoice) {
      setVoiceNotice("Selecione um agente e uma voz aprovada.");
      return;
    }
    const supportedModes = selectedVoice.delivery_modes.filter((mode): mode is "REALTIME_STREAM" | "MESSAGE_PLAYBACK" | "VOICE_MESSAGE" =>
      ["REALTIME_STREAM", "MESSAGE_PLAYBACK", "VOICE_MESSAGE"].includes(mode),
    );
    const confirmed = window.confirm(
      `Criar novo vínculo de voz em DRAFT para ${voiceAgentSlug}? O backend versionará/desativará o vínculo ativo anterior no mesmo locale, se existir.`,
    );
    if (!confirmed) return;
    setVoiceSaving(true);
    try {
      const assignment = await updateAdminAgentVoiceAssignment(voiceAgentSlug, {
        voice_catalog_id: selectedVoice.id,
        locale: voiceLocale.trim() || "pt-BR",
        delivery_modes: supportedModes,
        presentation_label: voicePresentation,
      });
      setVoiceAssignments((current) => [
        assignment,
        ...current.filter(
          (item) => !(item.agent_slug === assignment.agent_slug && item.locale === assignment.locale && item.assignment_state !== "DISABLED"),
        ),
      ]);
      setVoiceNotice(`Draft criado: ${assignment.agent_slug} → ${assignment.voice_display_name} (v${assignment.version}).`);
    } catch (err) {
      setVoiceNotice(err instanceof Error ? `Falha ao criar draft: ${err.message}` : "Falha ao criar draft de voz.");
    } finally {
      ...
      <section className="admin-ops__workspace">
        <header className="admin-ops__header">
          <div>
            ...
            <p>
              ...
              {view === "runtime" && "Liveness, readiness, release SHA e estado do banco deste deployment."}
        ...
        {loading ? (
          ...
          <>
                  ...
                  <article className="admin-panel admin-panel--wide">
                    <div className="admin-panel__heading">
                      ...
                      <div className="admin-domain-summary" aria-label="Estado dos contratos operacionais">
                        <span>Runtime <DomainBadge state={domainState.runtime} /></span>
                    ...
                    <div className="admin-status-grid">
                      <div><span>Ambiente</span><strong>{runtimeHealth?.environment || governance?.environment || "—"}</strong></div>
                      <div><span>Release SHA</span><strong className="admin-mono">{runtimeHealth?.sha ? runtimeHealth.sha.slice(0, 12) : governance?.release_sha?.slice(0, 12) || "—"}</strong></div>
                      <div><span>Readiness</span><strong>{runtimeReadiness?.status || "—"}</strong></div>
            ...
            {view === "voices" && (
              <div className="admin-view">
                <section className="admin-panel admin-voice-bind">
                  <div className="admin-panel__heading">
                    <div>
                      <span>VOICE ASSIGNMENT · ACTION_SUPPORTED</span>
                      <h2>Criar vínculo governado</h2>
                    </div>
                    <DomainBadge state={domainState.voices} />
                  </div>
                  <p>
                    Esta ação usa o contrato real <code>PUT /api/v2/admin/agents/&#123;agent_slug&#125;/voice-assignment</code>.
                    O backend cria sempre um novo vínculo em <strong>DRAFT</strong>, registra audit event e versiona o vínculo anterior do mesmo locale.
                  </p>
                  <div className="admin-action-grid">
                    <label>
                      <span>Agente</span>
                      <select value={voiceAgentSlug} onChange={(event) => setVoiceAgentSlug(event.target.value)}>
                        <option value="">Selecione…</option>
                        {agents.map((agent) => <option key={agent.slug} value={agent.slug}>{agent.display_name}</option>)}
                      </select>
                    </label>
                    <label>
                      <span>Voz aprovada</span>
                      <select value={voiceCatalogId} onChange={(event) => setVoiceCatalogId(event.target.value)}>
                        <option value="">Selecione…</option>
                        {eligibleVoices.map((voice) => (
                          <option key={voice.id} value={voice.id}>{voice.display_name} · {voice.provider_key}</option>
                        ))}
                      </select>
                    </label>
                    <label>
                      <span>Locale</span>
                      <input value={voiceLocale} onChange={(event) => setVoiceLocale(event.target.value)} maxLength={24} />
                    </label>
                    <label>
                      <span>Apresentação</span>
                      <select value={voicePresentation} onChange={(event) => setVoicePresentation(event.target.value as typeof voicePresentation)}>
                  ...
                  {selectedVoice && (
                    <div className="admin-action-preview">
                      ...
                      <strong>{selectedVoice.display_name}</strong>
                      <small>{selectedVoice.supported_locales.join(", ") || "locale não informado"} · {selectedVoice.delivery_modes.join(", ") || "delivery mode não informado"}</small>
                    </div>
                  )}
                  <div className="admin-action-footer">
                    <button
                      ...
                      disabled={voiceSaving || domainState.voices !== "ok" || !voiceAgentSlug || !voiceCatalogId}
                      onClick={() => void createVoiceDraft()}
                    >
                      {voiceSaving ? "Criando draft…" : "Criar vínculo em DRAFT"}
                    </button>
                    <small>Requer confirmação humana antes da chamada. Não ativa voz automaticamente.</small>
                  </div>
                  {voiceNotice && <div className="admin-action-notice" aria-live="polite">{voiceNotice}</div>}
                </section>

                <section className="admin-panel">
                  <span className="admin-eyebrow">VOICE CATALOG · GOVERNED</span>
                  <h2>Vozes disponíveis</h2>
                  <p>Somente vozes ativas e <code>APPROVED</code> aparecem no seletor operacional.</p>
                ...
                <section className="admin-panel">
                  <span className="admin-eyebrow">ASSOCIATIONS · VERSIONED</span>
                  <h2>Vínculos por agente</h2>
                  {voiceAssignments.length === 0 ? <p>Nenhuma associação criada.</p> : (
                    <div className="admin-team-grid">{voiceAssignments.map((assignment) => (
                      <article className="admin-panel" key={assignment.id}>
                        ...
                        <small>{assignment.locale} · versão {assignment.version}</small>
                      </article>
                    ))}</div>
                  )}
                </section>
            ...
            {view === "runtime" && (
              <div className="admin-view">
                <section className="admin-kpis admin-kpis--security">
                  <article><span>Liveness</span><strong className="admin-kpi-text">{runtimeHealth?.status || "—"}</strong></article>
                  <article><span>Readiness</span><strong className="admin-kpi-text">{runtimeReadiness?.status || "—"}</strong></article>
                  <article><span>Ambiente</span><strong className="admin-kpi-text">{runtimeHealth?.environment || "—"}</strong></article>
                  <article><span>Release</span><strong className="admin-kpi-text admin-mono">{runtimeHealth?.sha ? runtimeHealth.sha.slice(0, 12) : "—"}</strong></article>
                </section>
                {domainState.runtime !== "ok" && (
                  <div className="admin-domain-empty">
                    <DomainBadge state={domainState.runtime} />
                    <strong>Runtime status indisponível</strong>
                    <p>Não inferimos saúde a partir de outros domínios.</p>
                  </div>
                )}
                <section className="admin-panel">
                  <div className="admin-panel__heading">
                    <div><span>DATABASE / MIGRATION</span><h2>Readiness estrito</h2></div>
                    <DomainBadge state={domainState.runtime} />
                  </div>
                  <div className="admin-status-grid">
                    <div><span>Database connect</span><strong>{yesNo(runtimeReadiness?.checks?.database_connect as boolean | undefined)}</strong></div>
                    <div><span>Schema completo</span><strong>{yesNo(runtimeReadiness?.checks?.schema_complete as boolean | undefined)}</strong></div>
                    <div><span>Migration atual</span><strong>{yesNo(runtimeReadiness?.checks?.migration_current as boolean | undefined)}</strong></div>
                    <div><span>Migration head</span><strong className="admin-mono">{String(runtimeReadiness?.checks?.migration_head || "—")}</strong></div>
                    <div><span>Expected head</span><strong className="admin-mono">{String(runtimeReadiness?.checks?.migration_expected || "—")}</strong></div>
                    <div><span>Driver</span><strong>{String(runtimeReadiness?.checks?.driver || "—")}</strong></div>
                  </div>
                </section>
                <section className="admin-panel admin-governance-note">
                  <span className="admin-eyebrow">READ ONLY</span>
                  <h2>Observabilidade sem controle destrutivo</h2>
                  <p>
                    Esta superfície consulta somente <code>/api/v2/health</code> e <code>/api/v2/ready</code>.
                    Deploy, migration e restart permanecem fora deste painel até existir contrato governado e rollback comprovado.
                  </p>
                </section>
              </div>
            )}
